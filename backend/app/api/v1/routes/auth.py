...
def change_password(
    req: ChangePasswordRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)
) -> None:
    auth_service.change_password(db, current_user, req.new_password)
